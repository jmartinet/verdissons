...
class GenreResourceIT {
	...
	void createGenre() throws Exception {
		...
		restGenreMockMvc.perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON)
		...
		assertThat(testGenre.getLibelle()).isEqualTo(DEFAULT_NOM);
	}
	...
	void createGenreWithExistingId() throws Exception {
		...
		restGenreMockMvc.perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(genreDTO))).andExpect(status().isBadRequest());
	...
	void getAllGenres() throws Exception {
		...
		restGenreMockMvc.perform(get(ENTITY_API_URL + "?sort=id,desc")).andExpect(status().isOk())
	...
	void getGenre() throws Exception {
		...
		// Get the genre
		restGenreMockMvc.perform(get(ENTITY_API_URL_ID, genre.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
	...
	void putNewGenre() throws Exception {
		...
		// Disconnect from session so that the updates on updatedGenre are not directly
		// saved in db
		em.detach(updatedGenre);
		updatedGenre.setLibelle(UPDATED_NOM);
		GenreDTO genreDTO = genreMapper.toDto(updatedGenre);

		restGenreMockMvc.perform(put(ENTITY_API_URL_ID, genreDTO.getId()).contentType(MediaType.APPLICATION_JSON)
		...
		Genre testGenre = genreList.get(genreList.size() - 1);
		assertThat(testGenre.getLibelle()).isEqualTo(UPDATED_NOM);
	}
	...
	void putWithIdMismatchGenre() throws Exception {
		...
		restGenreMockMvc.perform(put(ENTITY_API_URL_ID, count.incrementAndGet()).contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(genreDTO))).andExpect(status().isBadRequest());
	...
	void putWithMissingIdPathParamGenre() throws Exception {
		...
		// If url ID doesn't match entity ID, it will throw BadRequestAlertException
		restGenreMockMvc.perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(genreDTO))).andExpect(status().isMethodNotAllowed());
	...
	void partialUpdateGenreWithPatch() throws Exception {
		...
		partialUpdatedGenre.setLibelle(UPDATED_NOM);

		restGenreMockMvc.perform(
				patch(ENTITY_API_URL_ID, partialUpdatedGenre.getId()).contentType("application/merge-patch+json")
	...
	void patchNonExistingGenre() throws Exception {
		...
		// If the entity doesn't have an ID, it will throw BadRequestAlertException
		restGenreMockMvc.perform(patch(ENTITY_API_URL_ID, genreDTO.getId()).contentType("application/merge-patch+json")
				.content(TestUtil.convertObjectToJsonBytes(genreDTO))).andExpect(status().isBadRequest());
	...
	void patchWithIdMismatchGenre() throws Exception {
		...
		// If url ID doesn't match entity ID, it will throw BadRequestAlertException
		restGenreMockMvc.perform(patch(ENTITY_API_URL_ID, count.incrementAndGet())
				.contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(genreDTO)))
	...
	void patchWithMissingIdPathParamGenre() throws Exception {
		...
		// If url ID doesn't match entity ID, it will throw BadRequestAlertException
		restGenreMockMvc.perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json")
				.content(TestUtil.convertObjectToJsonBytes(genreDTO))).andExpect(status().isMethodNotAllowed());
	...
	void deleteGenre() throws Exception {
		...
		// Delete the genre
		restGenreMockMvc.perform(delete(ENTITY_API_URL_ID, genre.getId()).accept(MediaType.APPLICATION_JSON))
